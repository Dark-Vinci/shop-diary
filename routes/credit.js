...
const idValidator = require('../middleware/bodyValidator');
...
router.put('/more-dept/:id', bodyAuthIdMiddleware, wrapper ( async ( req, res ) => {
    const shopId = req.shop._id;
    const { id } = req.params;
    ...
    if (shop.credit.indexOf(id) < 0) {
        ...
        if (!credit) {
            ...
            return res.status(404).json({
                status: 404,
                message: 'the credit been searched isnt in the db'
            });
        } else {
            //  the credit document exist and it is updated
            credit.set({
                products: { 
                    $push: { product, price, amount }
                }
            });
...
router.put('/pay-debt/:id', idAuthBody1, wrapper ( async ( req, res ) => {
    ...
    if (shop.credit.indexOf(id) < 0) {
        ...
        // when the credit id is stored in the shop document
        if (!credit) {
            // no such credit object in the database
            return res.status(404).json({
                status: 404,
                message: 'the credit been searched isnt in the db'
            });
...
router.delete('/:id', idAuthMiddleware, wrapper ( async ( req, res ) => {
    ...
    if (index < 0) {
        ...
        if (!credit) {
            // case, the credit document doesnt exist in the db
            return res.status(404).json({
                status: 404,
                message: 'the credit been searched isnt in the db'
            });
